...
from Exeption.EOF import EndOfBuffer
...
class LexemeAnalyzer:
    ...
    def lexemeAnalise(self) -> list[Lexeme]: # TODO: Change the logic of this method
        lexemes = []
        i = 0
            ...
            match self.string[i]:
                case "\\": # Doesn't work :3
                    if i + 1 < len(self.string):
                        lexemes.append(Lexeme(LexemeType.TEXT, self.string[i] + self.string[i+1]))
                        i += 1 # We need to do i += x less often
                    else:
                        lexemes.append(Lexeme(LexemeType.TEXT, self.string[i]))
                case "*":
                    if i + 1 < len(self.string) and self.string[i + 1] == "*":
                        lexemes.append(Lexeme(LexemeType.STRONG, self.string[i] + self.string[i + 1]))
                        i += 2
                    else:
                        lexemes.append(Lexeme(LexemeType.EMPHASIS, self.string[i]))
                        i += 1
                        continue
                case "_":
